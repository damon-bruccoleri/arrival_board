...
void config_from_env(AppConfig *cfg) {
    ...
    /* Pi Zero (not Zero 2): lighten GPU/CPU load — no background music or ferry overlay; 1080p via SDL (main.c). */
    if (util_is_pi_zero_v1()) {
        cfg->music_path[0] = '\0';
        cfg->music_loop2_path[0] = '\0';
        logf_("Pi Zero / Zero W: using 1080p fullscreen; background music and ferry sound disabled");
    }
}
